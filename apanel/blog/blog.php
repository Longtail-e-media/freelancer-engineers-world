...
if (isset($_GET['page']) && $_GET['page'] == "blog" && isset($_GET['mode']) && $_GET['mode'] == "list"):
    ...
    clearImages($moduleTablename, "blog/banner","other_upload");
    clearImages($moduleTablename, "blog/banner/thumbnails","other_upload");
    ?>
    <h3>
        List Blog
        ...
        <div class="example-code">
            <form action="" class="col-md-12 center-margin" id="blog_frm">
                ...
                <div class="form-row">
                    ...
                    <?php if (!empty($blogInfo->other_upload)): ?>
                        <div class="col-md-2" id="removeSavedimg7">
                            <div class="infobox info-bg">
                                <div class="button-group" data-toggle="buttons">
                            <span class="float-left">
                                <?php
                                if (file_exists(SITE_ROOT . "images/blog/banner/" . $blogInfo->other_upload)):
                                    $filesize = filesize(SITE_ROOT . "images/blog/banner/" . $blogInfo->other_upload);
                                    echo 'Size : ' . getFileFormattedSize($filesize);
                                endif;
                                ?>
                            </span>
                                ...
                                <img src="<?php echo IMAGE_PATH . 'blog/banner/thumbnails/' . $blogInfo->other_upload; ?>"
                                     style="width:100%"/>
                                <input type="hidden" name="imageArrayname7"
                                       value="<?php echo $blogInfo->other_upload; ?>"/>
                            </div>
                        </div>
                    <?php endif; ?>
                    <div class="form-input col-md-10 uploader7 <?php echo !empty($blogInfo->other_upload) ? "hide" : ""; ?>">
                        <input type="file" name="other_upload" id="other_upload" class="transparent no-shadow">
                        <label>
                        <small>Image Dimensions (1920 px
                            X 1280 px)
                        </small>
                    </label>
                    </div>
          ...
          // <![CDATA[
            ...
            $('#other_upload').uploadify({
                ...
                'onUploadSuccess': function (file, data, response) {
                    $('#uploadedImageName').val('1');
                    var filename = data;
                    $.post('<?php echo BASE_URL;?>apanel/blog/uploaded_banner.php', {imagefile: filename}, function (msg) {
