...
serclearImages($moduleTablename, "preference/facility", "facility_upload");
serclearImages($moduleTablename, "preference/facility/thumbnails", "facility_upload");
        ...
        <form action="" class="col-md-12 center-margin" id="Preference_frm">
            ...
            <div class="form-row hide">
                ...
                <?php if (!empty($PrefeRow->facility_upload)): 
                    $imgRec = unserialize($PrefeRow->facility_upload);
                   
                    if (is_array($imgRec)) {
                        foreach ($imgRec as $key => $recimg) {
                            $deleteid = rand(0, 99999);
                            $imagePath = SITE_ROOT . 'images/preference/facility/' . $recimg;
                            // pr();
                        ...
                        <span class="float-left">
                            <?php
                            if (file_exists(SITE_ROOT . "images/preference/facility/" . $recimg)):
                                $filesize = filesize(SITE_ROOT . "images/preference/facility/" . $recimg);
                                echo 'Size : ' . getFileFormattedSize($filesize);
                            endif;
                            ?>
                        </span>
                                            <a class="btn small float-right" href="javascript:void(0);"
                                               onclick="deleteSavedPreferenceimage(<?php echo $deleteid; ?>);">
                                                <i class="glyph-icon icon-trash-o"></i>
                                            </a>
                                        </div>
                                        <img src="<?php echo IMAGE_PATH . 'preference/facility/thumbnails/' . $recimg; ?>"
                                             style="width:100%"/>
                                        <input type="hidden" name="imageArrayname8[]" value="<?php echo $recimg; ?>"
        ...
        $('#facility_upload').uploadify({
            ...
            'onUploadSuccess': function (file, data, response) {
                ...
                $.post('<?php echo BASE_URL;?>apanel/preference/uploaded_facility_image.php', {imagefile: filename}, function (msg) {
                    $('#preview_Image8').append(msg).show();
                });
