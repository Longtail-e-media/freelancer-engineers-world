...
$resocl = $disicon = $dislink = '';
$http = 'https://www.';

if (!empty($socialRec)) {
    foreach ($socialRec as $socialRow) {
        $dislink = str_replace($http, '', $socialRow->linksrc);
        $icon = $socialRow->image;
        $disicon .= '
            <li>
                <a href="' . $socialRow->linksrc . '" class="text-decoration-none text-dark" target="_blank" rel="noopener noreferrer">
                    <img src="' . IMAGE_PATH . 'social/' . $socialRow->image . '" alt="' . $socialRow->title . '" class="footer-icons" />
                </a>
            </li>
        ';
    }
    $resocl .= '
        <ul class="footer-social-icons d-flex gap-3 list-unstyled">
            ' . $disicon . '
        </ul>
     ';
    $restop = $disicon;
...
$jVars['module:socilaLinktopmain'] = $restop;
...
if (!empty($otaRec)) {
    foreach ($otaRec as $otaRow) {
        $ota .= ' <li><a href="' . $otaRow->linksrc . '" target="_blank">' . $otaRow->title . '</a></li>';
    }
}
